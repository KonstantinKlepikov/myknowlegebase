...
Например здесь класс `Person` имеет два экземпляра дескриптора, `name` и `age`. Когда класс `Person` определяется, он выполняет обратный вызов `__set_name __()` в `LoggedAccess`, чтобы можно было определить имена полей, давая каждому дескриптору собственное имя `public_name` и `private_name`:
